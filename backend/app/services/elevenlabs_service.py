...
def _get_client() -> Optional[ElevenLabs]:
    """Returns a singleton ElevenLabs client, re-initializing if key changes.
    """
    ...
    if _client is None or api_key != _last_api_key:
        try:
            logger.info("Initializing new ElevenLabs client")
...
def _synthesize_speech_memoized(text: str, api_key: str, voice_id: str) -> str:
    """
    Internal memoized helper for TTS synthesis.
    Includes api_key and voice_id in arguments to ensure cache validity.
    """
    client = _get_client()
    if not client:
