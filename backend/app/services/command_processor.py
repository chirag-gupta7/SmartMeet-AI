...
# BOLT OPTIMIZATION: Pre-compile regular expressions at module scope to eliminate
# repeated regex compilation overhead on every voice command transcript evaluation.
...
_TODAY_EVENTS_PATTERN = re.compile(
...
_CALC_PATTERN = re.compile(r"\bcalculate\s+(.+)", re.IGNORECASE)
...
class VoiceCommandProcessor:
    ...
    def process_command(self, command: str, **kwargs) -> Dict[str, Any]:
        ...
        if command == 'unknown' and 'raw_command' in kwargs:
            ...
            if 'weather' in raw_command.lower():
                ...
                location_match = _WEATHER_LOCATION_PATTERN.search(
                    raw_command.lower()
                )
    ...
    def detect_and_process(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        if _TODAY_EVENTS_PATTERN.search(lowered) or (
        ...
        calc = _CALC_PATTERN.search(lowered)
        if calc:
