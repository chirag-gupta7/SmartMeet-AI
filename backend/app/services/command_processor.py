...
# BOLT OPTIMIZATION: Pre-compile regular expressions at module level to
# eliminate dynamic pattern compilation overhead during command processing.
_WEATHER_EXTRACT_PATTERN = re.compile(
...
_REMINDER_PATTERN = re.compile(
    r"\bremind me to\s+(?P<task>.+?)\s+"
    r"(?P<when>tomorrow|today|next \w+|at\s+.+|on\s+.+)$",
    re.IGNORECASE,
)
...
class VoiceCommandProcessor:
    ...
    def process_command(self, command: str, **kwargs) -> Dict[str, Any]:
        ...
        if command == 'unknown' and 'raw_command' in kwargs:
            ...
            if 'weather' in raw_command.lower():
                ...
                location_match = _WEATHER_EXTRACT_PATTERN.search(raw_command)
    ...
    def detect_and_process(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        note = _NOTE_PATTERN.search(text)
