...
def normalize_event(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        if start_val and "T" in start_val:
            # BOLT OPTIMIZATION: Use parse_iso_datetime from timeutils to avoid
            # unnecessary string allocations in Python 3.11+
        ...
        logger.warning(
            "Could not parse event times for %s: %s", event.get("id"), exc
        )
