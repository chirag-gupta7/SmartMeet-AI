...
# Global client to avoid re-instantiation
...
def _get_llm_response(user_text: str, api_key: str) -> str:
    """
    Internal memoized function to fetch LLM response.
    API key is passed to ensure cache is invalid if key changes.
    Raises Exception on failure so that errors are NOT cached.
    """
...
def generate_action_reply(user_text: str) -> Tuple[str, str]:
    """Generate action and reply from user text using LLM."""
    api_key = current_app.config.get("HUGGINGFACE_API_KEY")

    try:
        content = _get_llm_response(user_text, api_key)
