...
# BOLT OPTIMIZATION: Module-level tuple avoids list allocation and redundant
# .lower() calls on every natural language date parsing invocation.
_DAY_KEYWORDS = (
    "tomorrow",
    "today",
    "monday",
...
def parse_natural_language_datetime(text, timezone_name=None):
    ...
    # Note: `text` is already lowercased at function entry (`text = text.lower().strip()`),
    # so `keyword in text` is case-insensitive without needing redundant `.lower()` calls.
    if not time_found and not is_all_day:
        if any(keyword in text for keyword in _DAY_KEYWORDS):
            is_all_day = True
            base_date = base_date.replace(
    ...
    if not time_found and not day_signal_found and not any(
        keyword in text for keyword in _DAY_KEYWORDS
    ):
        err_msg = (
