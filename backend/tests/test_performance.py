...
def clear_caches():
    """Clear memoization caches around each test so results are deterministic."""
    llm_service._memoized_generate_action_reply.cache_clear()
    llm_service._hf_client = None
...
def test_llm_service_performance(app):
    with app.app_context():
        ...
        with patch('app.services.llm_service.InferenceClient') as mock_client_class:
            ...
            # First call (Miss)
            start = time.time()
            action, reply = llm_service.generate_action_reply("Hello")
            ...
            # Cached result must still be a parsed (action, reply) tuple
